...
func SaveDrone(drone *types.DroneData) error {
	...
	// 序列化协议特定数据
	var protocolDataJSON string
	if drone.Protocol == "gb46750" && drone.GB46750 != nil {
		if data, err := json.Marshal(drone.GB46750); err == nil {
			protocolDataJSON = string(data)
		}
	} else if drone.Protocol == "gb42590" && drone.GB42590 != nil {
		// 类似
	} else if drone.Protocol == "astm" && drone.ASTM != nil {
		// 类似
	}
...
func GetActiveDrones() ([]*types.DroneData, error) {
	...
	rows, err := db.QueryContext(ctx, `
        SELECT mac, uas_id, operator_id, ua_type, id_type, latitude, longitude, altitude,
               ...
               first_seen, last_seen, protocol_data
	...
	for rows.Next() {
		...
		var lat, lon, alt, spd, hdg, opLat, opLon sql.NullFloat64
		var fs, ls string
		var protocolData sql.NullString
		// ... 原有扫描
		...
		// 反序列化协议特定数据
		if protocolData.Valid && protocolData.String != "" {
			// 从 drone.Standard 推断协议类型
			if strings.Contains(d.Standard, "46750") {
				d.Protocol = "gb46750"
				d.GB46750 = &types.GB46750Data{}
				if err := json.Unmarshal([]byte(protocolData.String), d.GB46750); err != nil {
					slog.Warn("反序列化 GB46750 数据失败", "mac", d.MAC, "error", err)
				}
			} else if strings.Contains(d.Standard, "42590") {
				d.Protocol = "gb42590"
				d.GB42590 = &types.GB42590Data{}
				// 类似反序列化
			} else if strings.Contains(d.Standard, "ASTM") {
				d.Protocol = "astm"
				d.ASTM = &types.ASTMData{}
				// 类似反序列化
			}
		}
