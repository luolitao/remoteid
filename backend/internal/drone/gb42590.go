...
func (p *RemoteIDParser) decodeGBMessage(msgData []byte, msgType uint8) (string, map[string]any) {
	payload := msgData[1:]
...
func (p *RemoteIDParser) decodeGBLocation(payload []byte, data map[string]any) {
	if len(payload) < 22 {
		return
	}
...
func (p *RemoteIDParser) decodeGBSystem(payload []byte, data map[string]any) {
	if len(payload) < 18 {
		return
	}
