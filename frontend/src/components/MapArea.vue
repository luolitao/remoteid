...
<template>
  <div id="map_container">
    ...
    <!-- 右上角按钮组 -->
    <div id="header_side">
      <button class="toggle_sidebar_button" title="Hide map" @click="$emit('toggle-map')">
        <svg
          width="16"
          height="16"
          viewBox="0 0 24 24"
    ...
    <div v-if="selectedDrone" id="replayBar">
      <span class="font-bold" style="color: var(--TXTCOLOR1)">{{
        selectedDrone.uas_id || selectedDrone.mac
      }}</span>
      <input
      ...
      <button class="sidebarButton" style="font-size: 11px; padding: 1px 6px" @click="stopPlayback">
        ■
      </button>
      <button
        class="text-xs"
        style="color: var(--TXTCOLOR2)"
        @click="$emit('update:selectedDrone', null)"
      >
        ✕
      </button>
    </div>
...
const initMap = () => {
  ...
  map.value = L.map(mapContainer.value, { zoomControl: false, attributionControl: true }).setView(
    [23.14287, 113.26026],
    18,
  )
...
const updateMapLayer = () => {
  ...
  L.tileLayer(config.url, { subdomains: config.subdomains, attribution: config.attribution }).addTo(
    map.value,
  )
...
const showDroneTrajectory = async (mac) => {
  ...
  try {
    ...
    // 🎯 核心修复 1：严格的防御性校验
    // 防止后端返回 null、undefined 或非数组格式导致 points.length 报错中断循环
    if (!points || !Array.isArray(points) || points.length < 2) {
      logger.warn(`⚠️ 无人机 ${mac} 的轨迹数据不足 (点数: ${points ? points.length : 0})，跳过绘制`)
      return
    }

    // 🎯 核心修复 2：打印数据结构，方便您确认后端返回的字段名是否正确
    logger.info(`✅ 成功获取 ${mac} 的轨迹，样本点:`, points[0])

    // 提取经纬度 (请确保后端返回的字段名确实是 latitude 和 longitude)
    const latLngs = points.map((p) => [p.latitude, p.longitude])

    // 🎯 核心修复 3：Leaflet 的 color 属性无法正确解析 CSS 变量 'var(--ACCENT)'
    // 必须替换为具体的十六进制颜色值（例如 Tailwind 的 blue-600: #3182ce）
...
#map_canvas :deep(.leaflet-container) {
